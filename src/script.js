    ...
    let cardDesigns = data
      .map(
        ...
        <div class="cardDesign" >
          ...
          <div class="addToCartBtn" id="${item.id}" onclick= addtoCart('${item.id}')>
...
function addtoCart(getitemId){
  alert(getitemId);
  let htmlstrng = `<div class=>`
 fetch(("../data.json"))
  .then(response=>response.json())
  .then(data =>{
    const item = data.find(item => item.id === Number(getitemId)); // Convert getitemId to a number if id is a number
    if(item){
      console.log(item);
    }else{
